...
/* Registered as a WRAPPABLE core (not in the player list directly): the `chladni` graph doc wraps it
   with a `visual` node and drives its audio-reactive params (hue rates, switch sensitivity) from the
   node graph. hueRate/grainHueRate are now plain params (deg/s) — the graph decides what feeds them. */
...
/* Player-facing entry: wraps the core with a `visual` node and routes audio → its knobs through the
   graph. This is what you edit in the node editor — change these `map` nodes (or swap in an `lfo`,
   `spring`, etc.) to change what drives hue cycling / mode switching. Unwired core params (grain size,
   trail, migration…) stay on the panel as normal sliders. */
Synesthesia.Graph.register({
  id: "chladni",
  label: "Chladni Plate",
  nodes: [
    { id: "aud", type: "audio" },
    { id: "plateHue",   type: "map", in: { x: "aud:bass",   outMin: { const: 6 },   outMax: { const: 130 } } },
    { id: "grainHue",   type: "map", in: { x: "aud:treble", outMin: { const: 4 },   outMax: { const: 140 } } },
    { id: "modeSwitch", type: "map", in: { x: "aud:mid",    outMin: { const: 0.2 }, outMax: { const: 1 } } },
    { id: "viz", type: "visual", cfg: { ref: "chladniCore" },
      in: { hueRate: "plateHue", grainHueRate: "grainHue", switchSens: "modeSwitch" } }
  ]
});