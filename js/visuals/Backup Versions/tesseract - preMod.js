...
/* Registered as a WRAPPABLE core: the `tesseract` graph doc wraps it and exposes its knobs to the
   node graph (rotation gains, hue-cycle beats, energy swell, beat flash…) so you can define the
   audio-reactive links in the editor without touching the 4D rotation mechanics. */
...
/* Player-facing entry: wraps the core; the graph drives its params. Demo link: energy → saturation
   (desaturates in quiet passages). Edit/extend in the node editor — e.g. an `lfo` on `cycleBeats` for
   a shifting hue tempo, or `energy → scalePump`. Unwired knobs stay as panel sliders. */
Synesthesia.Graph.register({
  id: "tesseract",
  label: "Tesseract Drive",
  nodes: [
    { id: "aud", type: "audio" },
    { id: "sat", type: "map", in: { x: "aud:energy", outMin: { const: 45 }, outMax: { const: 100 } } },
    { id: "viz", type: "visual", cfg: { ref: "tesseractCore" }, in: { saturation: "sat" } }
  ]
});
